...
class MenuJeu(QtWidgets.QMainWindow, object):
    def __init__(self):
        super().__init__()
    ...
    def icone(self):
        ...
        icone.addPixmap(QtGui.QPixmap("logo_pokemon.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
    ...
    def nouveau(self, clicked = False):
        ...
        while running: #Tant que running ne passe pas à False (échap ou tous les pokémons capturés), le jeu continue
            fenêtre.fill(Blanc)  # Fond blanc, car le fond noir crée des bugs visuels
            ...
            if direction_dresseur is None:
                if dernière_direction_dresseur is not None:
                   direction_dresseur = dernière_direction_dresseur
                else:
            ...
            if direction_dresseur == 'gauche':
              fenêtre.blit(dresseur_gauche, (dresseur_x, dresseur_y))
            elif direction_dresseur == 'droite':
               fenêtre.blit(dresseur_droite, (dresseur_x, dresseur_y))
            elif direction_dresseur == 'haut':
               fenêtre.blit(dresseur_dos, (dresseur_x, dresseur_y))
            elif direction_dresseur == 'bas':
               fenêtre.blit(dresseur_face, (dresseur_x, dresseur_y))

            # On met à jour la dernière direction du dresseur
            dernière_direction_dresseur = direction_dresseur
            ...
            for i in range(len(tableau)): #On parcourt tous les pokémons et leurs coordonnées
                 ...
                 if Map([dresseur_x,dresseur_y], tableau,MIN_DISTANCE).distance([pokemon_x,pokemon_y],[dresseur_x,dresseur_y]) < MIN_DISTANCE:    
                 ...
                 if Map([dresseur_x,dresseur_y], tableau,MIN_DISTANCE).distance([pokemon_x,pokemon_y],[dresseur_x,dresseur_y]) < DISTANCE_INTERACTION:
                     ...
                     fenêtre.blit(boutons_combat[i], (pokemon_x + Taille_Pokemon[0] // 2 - bouton_combat_largeur // 2, pokemon_y - 20)) #On affiche le bouton de combat propre à ce pokémon via la liste de boutons, la position est choisie empiriquement pour être bien placée par rapport au pokémon
